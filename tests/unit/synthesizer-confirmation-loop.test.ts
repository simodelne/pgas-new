...
describe('confirmation_loop descriptor synthesis', () => {
  it('emits user_confirmation targeting, confirmation_pairing, propose_item, and an engine-valid spec', () => {
    ...
    expect(parsed.action_map.propose_item.mutations).toEqual(
      expect.arrayContaining([
        { op: 'MSet', path: 'work_units.items.*.title', value: '', from_arg: 'title' },
        { op: 'MSet', path: 'work_units.items.*.proposed_text', value: '', from_arg: 'proposed_text' },
        { op: 'MSet', path: 'work_units.items.*.status', value: 'proposed' },
      ]),
    );
  ...
  it('fails the pairing lint when a prefix-writing action is missing from terminals', () => {
    ...
    drifted.confirmation_pairing = {
      prefixes: ['work_units.items'],
      policy: 'reject',
      terminals: ['approve_item', 'revise_item', 'skip_item'],
    };

    expect(() => assertConfirmationPairingTerminals(drifted)).toThrow(/propose_item/u);
  });
...
function runEnforce(
  ...
  const result = enforce(snapshot, 'AfterMutation', 'review_work');
