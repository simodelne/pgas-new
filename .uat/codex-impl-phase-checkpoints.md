...
## Phase 3.17 - Disable forced JSON response_format for foundry LLM mode

- Commit: this commit (`fix(v3): Phase 3.17 — disable forced JSON response_format so Qwen emits native tool_calls (caught by §10)`)
- Verification: RED `npx vitest run --config tests/vitest.config.ts tests/unit/foundry-server.test.ts tests/integration/foundry-llm-mode.test.ts` failed on 2026-06-22 because `PGAS_OPENAI_DISABLE_JSON_RESPONSE_FORMAT` was `undefined`; GREEN focused run with `tests/unit/foundry-server.test.ts`, `tests/integration/foundry-llm-mode.test.ts`, and `tests/integration/foundry-tool-call-protocol.test.ts` PASS on 2026-06-22 with 11 tests passed / 2 listener-dependent tests skipped; `npm test` PASS on 2026-06-22 with 30 Vitest files / 180 tests passed and generated scaffold install/test SKIP because `NPM_TOKEN` is not explicitly set.
- Root cause: §10 Round 9 still saw Qwen emit legacy `MutationAction` content with `hadToolCalls=false` despite Phase 3.16's schema improvements. The installed `@simodelne/pgas-server` bundle forces `response_format: { type: "json_object" }` unless `PGAS_OPENAI_DISABLE_JSON_RESPONSE_FORMAT=1` is set; JSON mode plus available tools pulls Qwen back into content instead of the native `tool_calls` field.
- Notes: `startFoundryServer` now defaults `PGAS_OPENAI_DISABLE_JSON_RESPONSE_FORMAT=1` before `createPgasServer(...)`, while preserving an explicit override such as `=0`. `prompts.intake_intelligence` and `prompts.scaffold_plan` now lead with native tool-call protocol guidance: commit state with one OpenAI `tool_call`, do not emit JSON content or `MutationAction` objects, treat the spec mapping marker as internal syntax rather than an argument key, and ask user questions as plain natural-language content without a tool call.

## Phase 3.20 - Foundry proxy actually intercepts native tool calls

- Commit: this commit (`fix(v3): Phase 3.20 — proxy actually intercepts engine LLM calls (caught by §10 round 12)`).
- Verification: RED `npx vitest run --config tests/vitest.config.ts tests/integration/foundry-proxy-wire.test.ts` failed on 2026-06-22 because engine-originated OpenAI bodies had no native `tools` and retry conversation history omitted OpenAI `tool_calls[].type`. GREEN focused run `npx vitest run --config tests/vitest.config.ts tests/integration/foundry-tool-choice-proxy.test.ts tests/integration/foundry-proxy-wire.test.ts` PASS with 4 tests passed; `npm run typecheck` PASS. `npm test` PASS on 2026-06-22 with 34 Vitest files / 189 tests passed; static scaffold checks PASS; generated scaffold install/test SKIP because `NPM_TOKEN` is not explicitly set.
- Live diagnostic: tmux session `pgas-phase-3-20-diag` ran `PGAS_FOUNDRY_PROXY_DEBUG=1 PGAS_PROVIDER=openai PGAS_OPENAI_API_KEY=local PGAS_OPENAI_MODEL=qwen36-27b PGAS_OPENAI_DISABLE_THINKING=1 PGAS_OPENAI_BASE_URL=http://100.100.74.6:8000/v1 npm run pgas-new -- --slug diag-test --out /tmp/diag-test`, sent one line, and showed `[proxy] POST /v1/chat/completions`, `[proxy] -> http://100.100.74.6:8000/v1/chat/completions`, and outgoing body preview beginning with `{"tool_choice":"required",...}`. The round completed with a valid `choose_design_path` action.
- Root cause: Phase 3.19's proxy was reachable, but `startFoundryServer()` still launched the default JSON author path. That path sent prompted `messages` without native OpenAI `tools`, so the proxy's guarded injection never had a `tools` array to operate on and `hadToolCalls=false` persisted.
- Notes: `startFoundryServer()` now installs the PGAS unified native-tools driver for OpenAI-compatible foundry launches and supplies a small OpenAI-compatible completer that routes through the proxy URL in `PGAS_OPENAI_BASE_URL`. The proxy keeps `PGAS_FOUNDRY_PROXY_DEBUG=1` diagnostics gated off by default, redacts `Authorization`, and puts injected `tool_choice` first in the forwarded JSON so the 500-character preview proves the on-wire setting. `tests/integration/foundry-proxy-wire.test.ts` spins a fake vLLM, triggers a real foundry round, and asserts the fake upstream received native `tools` plus `tool_choice: "required"`; it also covers retry history preserving `type: "function"`.
