...
2. **Engine version pin** — default `^1.8.0` (matches the plugin's
   `engineCompatRange`). Used as the version range for every
...
- `{{ENGINE_VERSION}}` → the pgas version range (e.g. `^1.8.0`)
